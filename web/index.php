...
function insertRoom($Rooms, $freeAllDay, $room_number, $start_time, $end_time) {
	...
	foreach ($Rooms as $room_in_Rooms) { // Does room exist in $Rooms already?

    if ($room_number === $room_in_Rooms->getRoom()) {
			$roomExist = TRUE;

			if (!doubleBooked($room_in_Rooms, $start_time, $end_time)) { // if it's a double booking, we don't have to add it at all.
				$room_in_Rooms->addBookedTime($start_time, $end_time);

				while (canMerge($room_in_Rooms->getBooked()) !== false) { // Merge
	...
	if ($roomExist === FALSE) { // Does not exist in $Rooms -> new Room!
...
function doMerge($arr, $z) {
	 ...
	 */
function free($_booked) { // inverse of booked
	$free = [];
	$dasUberFree = [];
	
	for ($obj = 0; $obj < count($_booked); $obj++) { // obj
		$booked = $_booked[$obj]->getBooked();
		
		for ($i = 0, $ii = $i-1; $i < count($booked); $i++, $ii++) { // booked
			...
			if (($i > 0)) { // inbetween
				$free_time = substr($booked[$ii], 8, 5) . " - " . substr($booked[$i], 0, 5);
				array_push($free, $free_time);
			}

			if (($i === count($booked) - 1)) { // last element
...
// print_r2($_free_sorted); // RM
	if ($location === "pol") { // polacks
		echo "<div class='row'>";
			...
			$house2begin = -1;
			foreach($_free_sorted as $room_in_free_sorted) { // when do we encounter house 2?
				$house2begin++;
	...
	if ($location === "ang") { // ångan
		echo "<div class='row'>";
				...
				$corridor4begin = -1;
				foreach($_free_sorted as $room_in_free_sorted) { // when do we encounter corridor 4?
					$corridor4begin++;
...
*/
	 ...
	 */
function freeMain($location, $date) { // 1 freeMain
	$allRoomsInPol = ["1111", "1112", "1113", "1145", "1146", "1211", "1212", "1213", "1245", "1311", "1312", "1313", "2244", "2245", "2247", "2314", "2315", "2344"];
	$allRoomsInPolGrp = ["1149", "1152", "1154", "1156", "2226", "2232", "2240", "2241", "2242"];
	$allRoomsInAng = ["2001", "2002", "2003", "2004", "2005", "4001", "4003", "4004", "4005", "4006", "4007", "4101"]; // , "4104", "10131", "10133", "10202", "10204"
	$allRoomsInAngGrp = ["4102", "4103", "4104", "2040", "2041", "2042", "2043", "2044", "2045", "2046", "10131", "10133", "10202", "10204", "10211", "10212", "10213", "10214", "10205-07", "10208-10"];
	$allTheRooms = array_merge($allRoomsInPol, $allRoomsInAng, $allRoomsInPolGrp, $allRoomsInAngGrp);
	...
	if ($location === "pol") {
		...
		$file = fopen("https://se.timeedit.net/web/uu/db1/schema/ri1m0XYX65ZZ69Qm6Y5X2560y5Y59566206Q052Q9Y72Y6285590X05X99Y2226605Y968X206665262Y055Y7525Y692590XX2266152266955XY05296064X5Y6Y250X9552962X2906506503X50Y62560Y6537952X605Y6929Y9535862X56Y56X6503X02663059Y5XYb0Za76Z551672QfQc69x3coQ55.csv", 'r');
		$freeAllDay = $allRoomsInPol;
		}
	}
	...
	if ($location === "pol_grp") {
		if ($date == "today") {
			$file = fopen("https://se.timeedit.net/web/uu/db1/schema/ri1m6XYX85ZZ00Qm8Y0X5560y3Y50586968Q659Q0Y79Y6565502X05X94Y3356665Y968X606465366Y055Y7565Y699590XX73660555Z97x5Xc657QQ0cofQY6b3aZ7655.csv", 'r');		
			$freeAllDay = $allRoomsInPolGrp;			
		} else {
			$file = fopen("https://se.timeedit.net/web/uu/db1/schema/ri1m6XYX85ZZ00Qm8Y5X5560y3Y50586968Q659Q0Y79Y6565502X05X94Y3356665Y968X606465366Y055Y7565Y699590XX73660555Z97x5Xc657QQ0cofQY6b3aZ7655.csv", 'r');		
			$freeAllDay = $allRoomsInPolGrp;
		}
	...
	if ($location === "ang") {
		if ($date == "today") {
			$file = fopen("https://se.timeedit.net/web/uu/db1/schema/ri1m0XYX65ZZ19Qm6Y0X8560y3Y59566206Q052Q9Y72Y6885592X05X96Y2286605Y965X906465269Y055Y0595Y692590XX926675o86Q955XYZ59960617bY6c250X75f2Q62X990659Z5Q9ax6Yc55.csv", 'r');
			$freeAllDay = $allRoomsInAng;
		} else {
			$file = fopen("https://se.timeedit.net/web/uu/db1/schema/ri1m0XYX65ZZ19Qm6Y5X8560y3Y59566206Q052Q9Y72Y6885592X05X96Y2286605Y965X906465269Y055Y0595Y692590XX926675o86Q955XYZ59960617bY6c250X75f2Q62X990659Z5Q9ax6Yc55.csv", 'r');
			$freeAllDay = $allRoomsInAng;			
		}
	...
	if ($location === "ang_grp") {
		if ($date == "today") {
			$file = fopen("https://se.timeedit..csv", 'r');
			$freeAllDay = $allRoomsInAngGrp;		
		} else {
			$file = fopen("https://se.timeedit..csv", 'r');		
			$freeAllDay = $allRoomsInAngGrp;
		}
	...
	$i = 0;
	while (($line = fgetcsv($file)) !== FALSE) { // 2 while
// print_r2($line);
		if ($i <= 4) { $i++; } // jump over unimportant/empty line
		...
		if ($i >= 5) { // 3
			...
			while (strlen($room_number) > 3) { // Checks if first 4 characters are in $freeAllDay array.
...
// print_r2($single_room_number);
					if (in_array($single_room_number, $allTheRooms)) {
						$arrays = insertRoom($Rooms, $freeAllDay, $single_room_number, $start_time, $end_time); // returnerar $Rooms och $freeAllDay
// print_r2("ADDED: " . $single_room_number);
						$Rooms = $arrays[0];
						$freeAllDay = $arrays[1];
						$room_number = substr($room_number, $commaPos, strlen($room_number)); // Remove the room we just added
					} else {
						if ($commaPos == FALSE) {
							break; // If there's no more commas - and the current room is wrong. Break it.
						}
	//print_r2("COMMAPOS: " . $commaPos);
						$room_number = substr($room_number, $commaPos, strlen($room_number)); // Remove the room we can't match to our arrays with choosen rooms
...
/* echo "<div style='color:#fff;height:50px;width:100px;' id='angularTester'></div>"; */
	...
	echo "<div class='row dropdown top' ng-show=" . "btnctrl.getDate('today')" . ">"; //style='border:1px solid blue;'
	...
	echo 		"<button class='loc-btn pol-btn btn btn-default' ng-click=" . "btnctrl.changeLocation('pol')" . ">Polacks</button>";
	echo 		"<button class='loc-btn pol_grp-btn btn btn-default' ng-click=" . "btnctrl.changeLocation('pol_grp')" . ">Polacks, grupprum</button>";
	echo 		"<button class='loc-btn ang-btn btn btn-default' ng-click=" . "btnctrl.changeLocation('ang')" . ">Ångan</button>";
	echo 		"<button class='loc-btn ang_grp-btn btn btn-default' ng-click=" . "btnctrl.changeLocation('ang_grp')" . ">Ångan, grupprum</button>";
	echo 	"</ul>";
	echo "<i ng-click=btnctrl.setStarred() class='starred fa fa-star-o'></i>";
	
	echo "</div>";

	echo "<div class='row dropdown top' ng-show=" . "btnctrl.getDate('tomorrow')" . ">";
	echo 	"<button class='btn btn-default chooseCampusButton' id='dLabel' type='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>";
	echo 		"Välj område <span class='caret'></span>";
	echo  	"</button>";
	echo 	"<ul class='dropdown-menu filter-dropdowns' role='menu' aria-labelledby='dLabel'>";
	...
	echo 		"<button class='date-btn today-btn btn btn-default' ng-click=" . "btnctrl.changeDate('today')" . ">Idag (" . utf8_encode(strftime("%A")) . ")</button>"; //ucwords() for capital first letter
	echo 		"<button class='date-btn tomorrow-btn btn btn-default' ng-click=" . "btnctrl.changeDate('tomorrow')" . ">Imorgon (hint: inte ". utf8_encode(strftime("%A")) .")</button>";
	echo 	"</ul>";
...
$campuses = [];
$campuses[0] = "These are my campuses. They are very lovely. Om nom nom campuses. So many rooms. Lovely :)";
$oneHour = 1800;
	...
	<script>
		setTimeout(function() {
		    (function(h,o,t,j,a,r){
		        ...
		        h._hjSettings={hjid:352797,hjsv:5};
...
<!-- 	<?php include_once("../../hotjar.php"); ?> -->
<!-- 	<?php include_once("../../smartlook.php"); ?> -->
  </body>
</html>